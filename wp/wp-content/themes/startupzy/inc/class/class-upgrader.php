...
class Upgrader {
	...
	public function upgrader_index_close() {
		if ( isset( $_POST['nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'startupzy_upgrade_index' ) ) {
	...
	public function upgrader_page_close() {
		if ( isset( $_POST['nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'startupzy_upgrade_notice' ) ) {
	...
	public function upgrader_page_content_close() {
		if ( isset( $_POST['nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'startupzy_content_upgrade_notice' ) ) {
		...
		*/
		...
		if ( 'custom' === $front->source ) {
			wp_send_json(
				array(
					'flag'    => false,
					'message' => esc_html__( 'Front Page exist. If you want migrate index, please clear Front Page customization.', 'startupzy' ),
				)
			);
			exit;
		}
		...
		wp_insert_post(
			array(
				...
				'post_title'   => esc_html__( 'Front Page', 'startupzy' ),
	...
	public function index_notice() {
		...
		if ( $flag ) {
			...
			<div class="notice notice-important theme-upgrade-notice index-upgrade">
				<h2><?php esc_html_e( 'Important', 'startupzy' ); ?> <?php echo esc_html( wp_get_theme()->name ); ?> <?php esc_html_e( 'Upgrade Notice!', 'startupzy' ); ?></h2>
			<?php

			if ( 'custom' === $front->source ) {
				?>
					<p><?php esc_html_e( 'We notice you are upgrading from old theme and also have updated your Front Page. If previously you have issue with data lost on your home page, please proceed with upgrading process below.', 'startupzy' ); ?></p>
					<ol>
						<li><a href='<?php echo esc_url( admin_url( 'site-editor.php' ) ); ?>' target="_blank"><?php esc_html_e( 'Please visit site editor page.', 'startupzy' ); ?></a></li>
						<li><?php esc_html_e( 'Before resetting, please make sure you back up your changes of Front Page template first.', 'startupzy' ); ?></li>
						<li><?php esc_html_e( 'If you sure you are having this issue, you can', 'startupzy' ); ?>
							<a target="_blank" href="https://img001.prntscr.com/file/img001/h0PskNncSi2E9OnDRA36nQ.png"><?php esc_html_e( 'reset the Front Page template.', 'startupzy' ); ?></a>
							<?php esc_html_e( 'But don\'t reset the Index template.', 'startupzy' ); ?>
						</li>
						<li><?php esc_html_e( 'After resetting, please refresh this page.', 'startupzy' ); ?></li>
					</ol>
					<div class="themes-upgrade-action">
						<a class='button-primary close-notification' href="#"><?php esc_html_e( 'I don\'t have the issue', 'startupzy' ); ?></a>
					</div>
				<?php
			} else {
				?>
					<p><?php esc_html_e( 'We notice you are upgrading from old theme. If you have issue with data lost on your front page, please proceed with upgrading process below.', 'startupzy' ); ?></p>
					<ol>
						<li><a href='<?php echo esc_url( admin_url( 'site-editor.php' ) ); ?>' target="_blank"><?php esc_html_e( 'Please visit site editor page.', 'startupzy' ); ?></a></li>
						<li><?php esc_html_e( 'Check your front page (from top to bottom), if your front page looks different than before, please click "Fix Issue" button below.', 'startupzy' ); ?></li>
						<li><?php esc_html_e( 'This issue only happened for Front Page, any change you made elsewhere are still intact.', 'startupzy' ); ?></li>					
						<li><?php esc_html_e( 'This process will copy the content of your Index template to your Front Page template. You can always revert back to default front page by ', 'startupzy' ); ?>
							<a target="_blank" href="https://img001.prntscr.com/file/img001/h0PskNncSi2E9OnDRA36nQ.png"><?php esc_html_e( 'resetting the template.', 'startupzy' ); ?></a>
						</li>
					</ol>
					<div class="themes-upgrade-action">
						<a class='button-success upgrade-themes' href="#"><?php esc_html_e( 'Fix Issue', 'startupzy' ); ?></a>
						<a class='button-primary close-notification' href="#"><?php esc_html_e( 'I don\'t have the issue', 'startupzy' ); ?></a>
					</div>
				<?php
			}
			...
			<script>
				(function($) {
					$('.theme-upgrade-notice.index-upgrade .upgrade-themes').on('click', function() {
						...
						request.done(function(response) {
							if (response.flag) {
								...
								button.text('<?php esc_html_e( 'Fix Issue', 'startupzy' ); ?>');
							}
						})
					});
					...
					$('.theme-upgrade-notice.index-upgrade .close-notification').on('click', function( event ) {
						...
						var nonce = '<?php echo esc_html( wp_create_nonce( 'startupzy_upgrade_index' ) ); ?>';
				...
				if ( get_option( $this->get_page_content_option_name() ) ) {
					...
					<div class="notice notice-warning has-logo theme-upgrade-notice page-content-upgrade">
						...
						<div class="notice-content">
							<h2><?php echo sprintf( esc_html__( '%s Upgrade Notice!', 'startupzy' ), wp_get_theme()->name ); ?></h2>
							<p>
								<?php esc_html_e( 'There are some new exciting updates we want to announce. This update will required the latest version of Gutenverse plugin to work smoothly, so we recommend to update your Gutenverse plugin.', 'startupzy' ); ?>
							</p>
							<div class="themes-upgrade-action">
								<?php
								if ( 'plugins.php' !== $pagenow ) {
									?>
									<a class='button-primary upgrade-themes' href="<?php echo esc_url( admin_url( 'plugins.php' ) ); ?>"><?php esc_html_e( 'Go to plugin page', 'startupzy' ); ?></a>
									<?php
								}
								?>
								<a class='close-notification' href="#"><?php esc_html_e( 'Close notification', 'startupzy' ); ?></a>
							</div>
						</div>
					</div>
					<script>
						(function($) {
							$('.theme-upgrade-notice.page-content-upgrade .close-notification').on('click', function( event ) {
								event.preventDefault();
								var nonce = '<?php echo esc_html( wp_create_nonce( 'startupzy_content_upgrade_notice' ) ); ?>';
				...
				if ( get_option( $this->get_page_option_name() ) && ! ( 'admin.php' === $pagenow && isset( $_GET['page'] ) && 'gutenverse-upgrade-notice' === $_GET['page'] ) ) {
					...
					<div class="notice notice-warning has-logo theme-upgrade-notice page-upgrade">
						...
						<div class="notice-content">
							<h2><?php echo sprintf( esc_html__( '%s Upgrade Notice!', 'startupzy' ), wp_get_theme()->name ); ?></h2>
							<p>
								<?php esc_html_e( 'We\'ve made some changes to page content layout and global style. Please check ', 'startupzy' ); ?>
								<a target="_blank" href="<?php echo esc_url( admin_url( 'edit.php?post_type=page' ) ); ?>"><?php esc_html_e( 'your pages', 'startupzy' ); ?></a>
								<?php esc_html_e( ' to see how the new layout looks on your site. You can always revert back by enable the inherit layout option on the ', 'startupzy' ); ?>
								<a href="<?php echo esc_url( admin_url( 'admin.php?page=gutenverse-settings' ) ); ?>"><?php esc_html_e( 'Gutenverse settings.', 'startupzy' ); ?></a>
								<?php esc_html_e( ' dashboard. Learn more about new layout on ', 'startupzy' ); ?>
								<a href="<?php echo esc_url( admin_url( 'admin.php?page=gutenverse-upgrade-notice&version=1.6.0' ) ); ?>"><?php esc_html_e( 'this page.', 'startupzy' ); ?></a>
							</p>
							<div class="themes-upgrade-action">
								<a class='close-notification' href="#"><?php esc_html_e( 'Close notification', 'startupzy' ); ?></a>
								<a class="get-support" href='https://wordpress.org/support/plugin/gutenverse/' target="_blank">
								<?php esc_html_e( 'Got Question?', 'startupzy' ); ?>
								</a>
							</div>
						</div>
					</div>
					<script>
						(function($) {
							$('.theme-upgrade-notice.page-upgrade .close-notification').on('click', function(event) {
								event.preventDefault();
								var nonce = '<?php echo esc_html( wp_create_nonce( 'startupzy_upgrade_notice' ) ); ?>';
