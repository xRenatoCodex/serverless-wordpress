=== Startupzy ===
Contributors: Jegstudio
Requires at least: 5.9
Tested up to: 6.1
Requires PHP: 7.0
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html

== Description ==

Startupzy is a simple, responsive, and elegant theme template for WordPress fullsite editing and fully compatible with Gutenverse plugin. Startupzy's templates is built for people that have Tech Business, Startup, or Entrepreneur that need a beautiful landing page for their business. The templates includes both core version and Gutenverse plugin version, also has core and Gutenverse block patterns ready so you can start mix and match your template parts as you desire. The templates is built ready so you don't need to build it from scratch. We want to make your experience using WordPress fullsite editor more convenient.


== Copyright ==

Startupzy, 2022 Jegstudio
Startupzy is distributed under the terms of the GNU GPL.

This program is free software: you can redistribute it and/or modify
it under the terms of the GNU General Public License as published by
the Free Software Foundation, either version 2 of the License, or
(at your option) any later version.

This program is distributed in the hope that it will be useful,
but WITHOUT ANY WARRANTY; without even the implied warranty of
MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
GNU General Public License for more details.

Self designed images:
* Logo under the directory startupzy/assets/img/logo-startupzy.webp
Declaring these self designed images under GPL license version 2.0 =
License URL: http://www.gnu.org/licenses/gpl-2.0.html

Image for theme screenshot, Credit rawpixel.com
License: CC0 1.0 Universal (CC0 1.0)
License URL: https://pxhere.com/en/license
Source: https://pxhere.com/en/photo/1448357

Icon for block pattern,
License: (Icons: CC BY 4.0, Fonts: SIL OFL 1.1, Code: MIT License)
License URL: https://fontawesome.com/license/free

Image for block pattern, Credit Direct Media
License: CC0 1.0 Universal (CC0 1.0)
License URL: https://stocksnap.io/license
Source: https://stocksnap.io/photo/senior-business-UTEZRDTKPP 

Image  for block pattern, Credit rawpixel.com
License: CC0 1.0 Universal (CC0 1.0)
License URL: https://pxhere.com/en/license
Source: https://pxhere.com/en/photo/1448357

Image  for block pattern, Credit rawpixel.com
License: CC0 1.0 Universal (CC0 1.0)
License URL: https://pxhere.com/en/license
Source: https://pxhere.com/en/photo/1442007

Image  for block pattern, Credit Q000024
License: CC0 1.0 Universal (CC0 1.0)
License URL: https://pxhere.com/en/license
Source: https://pxhere.com/en/photo/1629587

Image  for block pattern, Credit rawpixel.com
License: CC0 1.0 Universal (CC0 1.0)
License URL: https://pxhere.com/en/license
Source: https://pxhere.com/en/photo/1452617

Image  for block pattern, Credit tejipta
License: CC0 1.0 Universal (CC0 1.0)
License URL: https://pxhere.com/en/license
Source: https://pxhere.com/en/photo/1573329

Image  for block pattern, Credit tejipta
License: CC0 1.0 Universal (CC0 1.0)
License URL: https://pxhere.com/en/license
Source: https://pxhere.com/en/photo/1573331

Image  for block pattern
License: CC0 1.0 Universal (CC0 1.0)
License URL: https://pxhere.com/en/license
Source: https://pxhere.com/en/photo/1177664

Image  for block pattern
License: CC0 1.0 Universal (CC0 1.0)
License URL: https://pxhere.com/en/license
Source: https://pxhere.com/en/photo/108386

Image  for block pattern
License: CC0 1.0 Universal (CC0 1.0)
License URL: https://pxhere.com/en/license
Source: https://pxhere.com/en/photo/1172571

Image  for block pattern, Credit rawpixel.com
License: CC0 1.0 Universal (CC0 1.0)
License URL: https://pxhere.com/en/license
Source: https://pxhere.com/en/photo/1436177

Image  for block pattern, Credit rawpixel.com
License: CC0 1.0 Universal (CC0 1.0)
License URL: https://pxhere.com/en/license
Source: https://pxhere.com/en/photo/1449315


== Changelog ==

= 1.1.4 - 2024-05-17 =
* Update Gutenverse template path
* Remove unused core templates

= 1.1.3 - 2024-03-21 =
* Update notice design

= 1.1.2 - 2024-01-31 =
* Update Ajax

= 1.1.1 - 2023-07-11 =
* Update theme dashboard
* Fix post content inherit layout
* Improve the loading of font assets.

= 1.1.0 - 2022-11-16 =
* Use global fonts and colors in templates
* Update page templates
* Add new custom templates : blank canvas, and basic template

= 1.0.9 - 2022-10-14 =
* Add solution option to fix missing home page template

= 1.0.8 - 2022-09-14 =
* Add new gutenverse templates for 404, archive, index, page, and search
* Fix single post template

= 1.0.7 - 2022-08-18 =
* Fix gutenverse templates load mechanism
* Update gutenverse template animation

= 1.0.6 - 2022-07-28 =
* Fix animation load issue

= 1.0.5 - 2022-07-07 =
* Add new front page, archive, and search core templates
* Rework core templates
* Update animations
* Fix colors

= 1.0.4 - 2022-07-01 =
* Host theme's webfont locally
* Fix excerpt on admin pages
* Update theme's dashboard page
* Small fix gutenverse patterns
* Support WordPress up to v6.0

= 1.0.3 - 2022-06-02 =
* Add a new FAQ template for gutenverse version

= 1.0.2 - 2022-05-21 =
* Small fix heading def color css

= 1.0.1 - 2022-05-17 =
* Fix pattern image url
* Fix navigation submenu text color
* Fix navigation close button unclickable on mobile when user is logged in
* Fix skip link & keyboard navigation

= 1.0.0 - 2022-05-09 =
* Initial release